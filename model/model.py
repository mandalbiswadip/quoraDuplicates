...
class Model:
    ...
    def add_lstm_op(self):
        ...
        with tf.variable_scope('lstm'):
            ...
            (_,_), (state_one_fw, state_one_bw) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs=self.question_one,
                                                   sequence_length=self.seql_one, dtype=tf.float32)
            ...
            (_, _), (state_two_fw, state_two_bw) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs=self.question_two,
                                                                    sequence_length=self.seql_two, dtype=tf.float32)
    ...
    def add_logit_op(self):
        with tf.variable_scope('proj'):
            ...
            W = tf.get_variable('W', dtype=tf.float32,
                            shape=[4*config.n_hidden, config.n_tags], initializer=tf.truncated_normal_initializer())
    ...
    def run_epoch(self, train_data, dev_data, epoch):
        ...
        t_loss = 0
        for i, (sent_1, sent_2, sent_1_lengths, sent_2_lengths, label) in enumerate(minibatches(train_data, config.batch_size)):
        ...
        c = 0
        tot_ac = 0
        for i, (dev_texts_1, dev_texts_2, dev_texts_lens_1, dev_texts_lens_2, dev_label) in enumerate(minibatches(dev_data, config.batch_size)):
